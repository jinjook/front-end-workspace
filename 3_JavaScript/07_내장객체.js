...
function btn4() {
  ...
  console.log("월 : ", now.getMonth() + 1); // 5, 0~11 로 계산됨 +1 해줘야함
  console.log("일 : ", now.getDate()); // 12
  console.log("시 : ", now.getHours()); // 12
  console.log("분 : ", now.getMinutes()); // 15
  console.log("초 : ", now.getSeconds()); // 56
}

function btn5() {
  // 년, 월, 일, 시, 분, 초
  const a = new Date(2024, 10, 13, 12, 50, 0); // 동일하게 월은 -1 해야함
  console.log(a);
...
function btn8() {
  ...
  location.href = "https://naver.com"; // 해당 페이지로 이동
}
