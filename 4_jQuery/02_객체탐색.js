// 1. Filtering 메서드
// first : 선택된 요소들 중 첫 번재 요소 선택
$("h4").first().css("font-size", "4rem");
$("h4:first").css("color", "indianred");
$("h4").filter("first").css("background-color", "beige");

// last : 선택된 요소들 중 마지막번째 요소 선택
$("h4:last").css("font-size", "3rem");

// eq(n) : 선택된 요소들 중 n번째 요소 선택 (0부터 시작)
// 3번째 요소를 가져와서 "eq에 의해 선택되었습니다" 로 텍스트 변경
$("h4:eq(2)").text("eq에 의해 선택되었습니다.");

// odd : 홀수번째 / even : 짝수번째 요소 선택
// 홀수번째 인덱스 요소를 가져와서 배경색 : darkgreen, 글자색 : white 변경
$("h4:odd").css({ "background-color": "darkgreen", color: "white" });
...
$("span").parent().css({ color: "crimson", border: "2px solid" });

//parents(['선택자']) : 선택된 요소의 모든 상위 요소를 선택하거나, 매개변수가 있으면 매개변수와 일치하는 상위 요소만 선택
// li 요소의 모든 상위 요소의 글자색 : navy
$("li").parents().css("color", "navy");
// li 요소의 상위 요소 중 div만 테두리 : 2px dashed - tomato
$("li").parents("div").css("border", "2px dashed tomato");

// parentsUntil(['선택자']) : 선택된 요소의 모든 사우이 요소를 선택하거나, 매개변수가 있으면 선택된 요소부터 매개변수와 일치하는 요소까지의 상위 요소 선택
// span 요소부터 div 요소 이전까지 상위 요소에 배경색 : beige
$("span").parentsUntil("div").css("background-color", " beige");

// children(['선택자']) : 선택된 요소의 모든 자식 요소를 선택하거나 매개변수가 있으면 매개변수와 일치하는 자식 요소만 선택
console.log($(".wrap").children()); // div, div
//find('선택자') : 선택된 요소의 하위 요소 중 매개변수와 일치하는 모든 하위 요소 선택
console.log($(".wrap").find("span"));

let style1 = { color: "green", border: "2px solid" };
let style2 = { color: "deeppink", border: "2px solid" };
let style3 = { color: "grey", border: "2px solid" };
let style4 = { color: "plum", border: "2px solid" };
...
// h2 요소의 다음에 오는 모든 형제 요소에 배경색 : lavender
$(".wrap2 h2").nextAll().css("background", "lavender");
