...
- spce-between: 아이템들 사이에 균일한 간격으로 정렬 (0:1:1:0)
- space-around: 각 아이템의 좌우에 균일한 간격으로 정렬 (1:2:2:1)
- space-evenly: 아이템들의 사이와 양 끝에 균일한 간격으로 정렬 (1:1:1:1)
...
- 여러 행 정렬
- `flex-wrap: wrap / wrap-reverse;` 이 설정된 상태에서, 아이템들의 행이 2줄 이상인 경우 수직축 방향 정렬을 결정하는 속성
...
- width처럼 사이즈 조정, but 창 크기를 줄이면 줄이는대로 사이즈 조절됨
...
- flex-grow, flex-shirnk, flex-basis를 한번에 쓸 수 있는 속성
